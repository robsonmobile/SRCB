...
while($DSAC=mysql_fetch_array($SAC)){
    ...
    if(mysql_num_rows($SABC)>=1){
        ...
        $pdf->Cell (34,6,utf8_decode('CÓDIGO'),1,0,'C',true);
        $pdf->Cell (100,6,utf8_decode('TÍTULO'),1,0,'C',true);
        $pdf->Cell (65,6,utf8_decode('AUTOR'),1,0,'C',true);
        $pdf->Cell (35,6,utf8_decode('PAÍS'),1,0,'C',true);
        $pdf->Cell (16,6,utf8_decode('PREC.'),1,0,'C',true);
        $pdf->Cell (14,6,utf8_decode('CAN.'),1,0,'C',true);
        $pdf->Cell (16,6,utf8_decode('TOTAL'),1,0,'C',true);
        $pdf->Ln(6);
        $pdf->SetFont("Times","",10);
        while($DSABC=mysql_fetch_array($SABC)){
            $bookCode=$DSABC['CodigoInfraestructura'].'-'.$DSABC['CodigoCategoria'].'-'.$DSABC['CodigoCorrelativo'].'1/'.$DSABC['Existencias'];
            $PriceT=$DSABC['Estimado']*$DSABC['Existencias'];
            $pdf->Cell (34,6,utf8_decode($bookCode),1,0,'C');
            ...
            $pdf->Cell (16,6,utf8_decode('$'.$DSABC['Estimado']),1,0,'C');
    ...
    mysql_free_result($SABC);
}
mysql_free_result($SAC);
